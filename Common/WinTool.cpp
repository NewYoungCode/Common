...
#define FREE(x) HeapFree(GetProcessHeap(), 0, (x))
	...
	void SaveDCtoBMP(HDC hdc, const RECT& rect, const std::wstring& filename)
	{
		// 获取DC的宽度和高度
		...
		bmfh.bfSize = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + width * height * 3; // 文件大小
		bmfh.bfReserved1 = 0;
		bmfh.bfReserved2 = 0;
		bmfh.bfOffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER); // 位图数据偏移量

		// 打开文件并写入位图数据
		HANDLE fileHandle = ::CreateFileW(filename.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (fileHandle != INVALID_HANDLE_VALUE)
		{
			DWORD bytesWritten;
			::WriteFile(fileHandle, &bmfh, sizeof(BITMAPFILEHEADER), &bytesWritten, NULL);
			::WriteFile(fileHandle, &bi, sizeof(BITMAPINFOHEADER), &bytesWritten, NULL);
			BYTE* buffer = new BYTE[width * height * 3];
			::GetDIBits(memDC, bitmap, 0, height, buffer, (BITMAPINFO*)&bi, DIB_RGB_COLORS);
			::WriteFile(fileHandle, buffer, width * height * 3, &bytesWritten, NULL);
